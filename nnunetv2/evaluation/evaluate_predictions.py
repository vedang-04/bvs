...
def compute_metrics(reference_file: str, prediction_file: str, image_reader_writer: BaseReaderWriter,
                    ...
                    ignore_label: int = None, is_spare: bool = False) -> dict:
    # for i in range(len(reference_file)):
    #     # load images
    #     seg_ref, seg_ref_dict = image_reader_writer[i].read_seg(reference_file[i])
    #     idx = np.where(seg_ref==3)
    #     seg_ref[idx] = 1.0
    #     idx = np.where(seg_ref == 2)
    #     seg_ref[idx] = 0.0
    #     seg_pred, seg_pred_dict = image_reader_writer[i].read_seg(prediction_file[i])
...
def compute_metrics_on_folder(folder_ref: str, folder_pred: str, output_file: str,
                              ...
                              chill: bool = True, is_spare: bool = False) -> dict:
    """
    output_file must end with .json; can be None
    """
    if is_spare: #limiting for vents
        regions_or_labels = regions_or_labels[:1]
    ...
    with multiprocessing.get_context("spawn").Pool(num_processes) as pool:
        results = pool.starmap(
            ...
            list(zip(files_ref, files_pred, [image_reader_writer] * len(files_pred), [regions_or_labels] * len(files_pred),
                     [ignore_label] * len(files_pred), [is_spare] * len(files_pred)))
        )
