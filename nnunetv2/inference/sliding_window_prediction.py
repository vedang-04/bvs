...
def maybe_mirror_and_predict(network: nn.Module, x: torch.Tensor, mirror_axes: Tuple[int, ...] = None) \
        -> torch.Tensor:
    prediction = network(x) #inference output
    flg = 0
    ...
    if flg:
        ...
        if mirror_axes is not None:
            ...
            if 0 in mirror_axes:
                prediction += torch.flip(network(torch.flip(x, (2,)))[-1], (2,))
            if 1 in mirror_axes:
                prediction += torch.flip(network(torch.flip(x, (3,)))[-1], (3,))
            if 2 in mirror_axes:
                prediction += torch.flip(network(torch.flip(x, (4,)))[-1], (4,))
            if 0 in mirror_axes and 1 in mirror_axes:
                prediction += torch.flip(network(torch.flip(x, (2, 3)))[-1], (2, 3))
            if 0 in mirror_axes and 2 in mirror_axes:
                prediction += torch.flip(network(torch.flip(x, (2, 4)))[-1], (2, 4))
            if 1 in mirror_axes and 2 in mirror_axes:
                prediction += torch.flip(network(torch.flip(x, (3, 4)))[-1], (3, 4))
            if 0 in mirror_axes and 1 in mirror_axes and 2 in mirror_axes:
                prediction += torch.flip(network(torch.flip(x, (2, 3, 4)))[-1], (2, 3, 4))
...
def predict_sliding_window_return_logits(network: nn.Module,
                                         ...
                                         spare: bool = False) -> Union[np.ndarray, torch.Tensor]:
