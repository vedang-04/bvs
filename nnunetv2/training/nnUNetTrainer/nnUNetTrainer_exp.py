...
class MyEnsemble(nn.Module):
    ...
    def forward(self, x1):
        x2 = self.modelA(x1) # x2 is list during training and tensor while inference. Segmentation logits stage 1 # torch.Size([2, 3, 16, 320, 320]), torch.Size([2, 3, 16, 160, 160]), torch.Size([2, 3, 16, 80, 80]), torch.Size([2, 3, 16, 40, 40]), torch.Size([2, 3, 8, 20, 20]), torch.Size([2, 3, 4, 10, 10])
        if type(x2)==list:
            x22 = x2[0]
        else:
            x22 = x2.clone()
        x31 = torch.cat([x1,x22], axis=1) # stacking with data to create a 4 channel input. # torch.Size([2, 4, 16, 320, 320])
        x3 = self.modelB(x31)
        if type(x3)==list: # x3 is list (list of residuals) during training and tensor while inference. # torch.Size([2, 2, 16, 320, 320]), torch.Size([2, 2, 16, 160, 160]), torch.Size([2, 2, 16, 80, 80]), torch.Size([2, 2, 16, 40, 40]), torch.Size([2, 2, 8, 20, 20]), torch.Size([2, 2, 4, 10, 10])
            op_loss = []
            for i in range(len(x3)):
                op_loss.append(x3[i] + x2[i][:,:2]) # combining bg and vent from modelA and modelB. temp logits leading to Segmentation logits stage 2
        else:
            op_loss = x3 + x2[:,:2] # combining bg and vent from modelA and modelB
        return [x2, x3], op_loss # returning output of modelA, modelB and combined output for loss


class nnUNetTrainer_exp(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda'), spare_dataset_json: list = []):
        ...
        self.spare_dataset_json = spare_dataset_json
        if len(spare_dataset_json):
            self.is_spare = True
        else:
            self.is_spare = False
        self.fold = fold
        ...
        print('initial_lr: {}, weight_decay: {}, oversample_foreground_percent: {}, num_iterations_per_epoch: {}, '
              'num_val_iterations_per_epoch: {}, self.num_epochs: {}, self.current_epoch: {}'
              .format(self.initial_lr, self.weight_decay, self.oversample_foreground_percent,
                      self.num_iterations_per_epoch, self.num_val_iterations_per_epoch, self.num_epochs,
                      self.current_epoch)
              )
    ...
    def initialize(self):
        if not self.was_initialized:
            self.num_input_channels = determine_num_input_channels(self.plans_manager, self.configuration_manager,
                                                                   self.dataset_json)
            # self.network_0 = self.build_network_architecture(self.plans_manager, self.dataset_json,
            #                                                self.configuration_manager,
            #                                                self.num_input_channels,
            #                                                enable_deep_supervision=True).to(self.device)
            assert len(self.spare_dataset_json)==2
            self.network_1 = self.build_network_architecture(self.plans_manager, self.spare_dataset_json[0],
                                                             self.configuration_manager,
                                                             self.num_input_channels, # input channel number = 1
                                                             enable_deep_supervision=True).to(self.device)
            self.network_2 = self.build_network_architecture(self.plans_manager, self.spare_dataset_json[1],
                                                             self.configuration_manager,
                                                             self.num_input_channels * 4, # input channel number = 4
                                                             enable_deep_supervision=True).to(self.device)
            self.network = MyEnsemble(self.network_1, self.network_2)
    ...
    def set_deep_supervision_enabled(self, enabled: bool):
        ...
        if self.is_ddp:
            ...
            self.network.modelA.decoder.deep_supervision = enabled
            self.network.modelB.decoder.deep_supervision = enabled
    ...
    def convert_target_1(self, target_f):
        target_ = target_f.copy()
        for sm in target_:
            sm[(sm == 3)] = float(2)
        return target_

    def convert_target_2(self, target_f):
        target_ = target_f.copy()
        for sm in target_:
            sm[(sm == 2)] = float(0)
            sm[(sm == 3)] = float(1)
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            [output_1, output_2], loss_output = self.network(data) #output from wrapper
            # del data
            #stage1
            target1 = self.convert_target_1(target) # converting target for stage 1 cls: bg, v, b; b = b+bv
            target2 = self.convert_target_2(target) # converting target for stage 2 cls: bg, v; bg = b+bg, v=bv+v
            # combining in wrapper
            l1 = self.loss(output_1, target1)
            l2 = self.loss(loss_output, target2) # loss as mentioned in part 2 of pipeline with added seg logits
            l = 1.0 * l1 + 1.0 * l2 #net stage1 and stage2 loss
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        # If the device_type is 'mps' then it will complain that mps is not implemented, even if enabled=False is set. Whyyyyyyy. (this is why we don't make use of enabled=False)
        # So autocast will only be active if we have a cuda device.
        target_stage1 = target.copy()
        target_stage2 = target.copy()
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            [output_1, output_2], loss_output = self.network(data)

            target1 = self.convert_target_1(target_stage1)
            l1 = self.loss(output_1, target1)

            target2 = self.convert_target_2(target_stage2)
            l2 = self.loss(loss_output, target2)
            del data
            l = 1.0 * l1 + 1.0 * l2

            output_2 = loss_output[0] # combined output. no need for individual predicted_seg_onehot addition as output is a sum
            target2 = target2[0]
            axes = [0] + list(range(2, len(output_2.shape)))

            if self.label_manager.has_regions:
                predicted_segmentation_onehot = (torch.sigmoid(output_2) > 0.5).long()
            else:
                # no need for softmax
                output_2_seg = output_2.argmax(1)[:, None]
                predicted_segmentation_onehot = torch.zeros(output_2.shape, device=output_2.device, dtype=torch.float32)
                predicted_segmentation_onehot.scatter_(1, output_2_seg, 1)
                del output_2_seg

            if self.label_manager.has_ignore_label:
                if not self.label_manager.has_regions:
                    mask2 = (target2 != self.label_manager.ignore_label).float()
            ...
            tp, fp, fn, _ = get_tp_fp_fn_tn(predicted_segmentation_onehot, target2, axes=axes, mask=mask2) # metrics for combined output
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        if len(self.spare_dataset_json):
            num_seg_heads = num_seg_heads // 2 # converting to vent class

        inference_gaussian = torch.from_numpy(compute_gaussian(self.configuration_manager.patch_size, sigma_scale=1. / 8))
                    ...
                    segmentation_export_pool.starmap_async(
                        export_prediction_from_softmax, (
                            (prediction_for_export, properties, self.configuration_manager, self.plans_manager,
                             self.dataset_json, output_filename_truncated, save_probabilities, self.is_spare),
                        )
                    )
                )
                # for debug purposes
        ...
        if self.local_rank == 0:
            metrics = compute_metrics_on_folder(join(self.preprocessed_dataset_folder_base, 'gt_segmentations'),
                                                ...
                                                self.label_manager.ignore_label, chill=True, is_spare=self.is_spare)
