...
class LabelManager(object):
    ...
    def convert_probabilities_to_segmentation(self, predicted_probabilities: Union[np.ndarray, torch.Tensor], spare: bool = False) -> \
            Union[np.ndarray, torch.Tensor]:
        """
        ...
        if not spare:
            ...
            assert predicted_probabilities.shape[0] == (self.num_segmentation_heads)//2, \
                f'unexpected number of channels in predicted_probabilities. Expected {self.num_segmentation_heads}, ' \
                f'got {predicted_probabilities.shape[0]}. Remeber that predicted_probabilities should have shape ' \
                f'(c, x, y(, z)).'
        if self.has_regions:
    ...
    def convert_logits_to_segmentation(self, predicted_logits: Union[np.ndarray, torch.Tensor], spare: bool = False) -> \
            Union[np.ndarray, torch.Tensor]:
        probabilities = self.apply_inference_nonlin(predicted_logits)
        return self.convert_probabilities_to_segmentation(probabilities, spare)
